...
import type { CbmArgs, CbmCallOptions, CbmCommandResult, CbmTransport } from "./types.js";
...
export class PersistentCbmTransport implements CbmTransport {
  ...
  async callTool<T = unknown>(tool: string, args: CbmArgs = {}, options: CbmCallOptions = {}): Promise<CbmCommandResult<T>> {
    await this.ensureInitialized();
    const startedAt = performance.now();
    const response = await this.request("tools/call", { name: tool, arguments: toMcpArgs(args) }, options, tool);
  ...
  private request(
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Codebase Memory MCP request timed out: ${timeoutLabel}`));
      }, options.timeoutMs ?? this.requestTimeoutMs);
...
export class FallbackCbmTransport implements CbmTransport {
  ...
  async callTool<T = unknown>(tool: string, args: CbmArgs = {}, options: CbmCallOptions = {}): Promise<CbmCommandResult<T>> {
    if (this.usingFallback) {
      return this.fallback.callTool<T>(tool, args, options);
    }
    try {
      return await this.primary.callTool<T>(tool, args, options);
    } catch (error) {
      if (options.timeoutMs !== undefined && error instanceof Error && /timed out/.test(error.message)) {
        throw error;
      }
      this.usingFallback = true;
      await this.primary.close();
      const result = await this.fallback.callTool<T>(tool, args, options);
